	...
	if (view == 'list')
	{
		...
		$('.product_list > li').each(function(index, element) {
			...
			html = '<div class="product-container"><div class="row">';
				html += '<div class="item-left col-sm-4 col-md-3 col-sms-4 col-smb-12"><div class="item-left-inner">';
					html += '<div class="item-inner-top">' + $(element).find('.item-inner-top').html() + '</div>';
					html += '<ul class="icon">'+ $(element).find('.icon').html() + '</ul>';
				html += '</div></div>';
				html += '<div class="item-right col-sm-8 col-md-9 col-sms-8 col-smb-12">';
					html += '<h5 itemprop="name" class="product-name">'+ $(element).find('h5').html() + '</h5>';
					var rating = $(element).find('.comments_note').html(); // check : rating
					if (rating != null) { 
						html += '<div itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating" class="comments_note">'+ rating + '</div>';
					}
					var price = $(element).find('.content_price.left').html(); // check : catalog mode is enabled
					if (price != null) { 
